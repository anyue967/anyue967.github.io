...
- [监控服务](#监控服务)
  - [1. 常见的监控平台](#1-常见的监控平台)
    - [1.1 Cacti 流量与性能监测为主 Cacti](#11-cacti-流量与性能监测为主-cacti)
    - [1.2 Nagios 服务与性能检测为主 Nagios](#12-nagios-服务与性能检测为主-nagios)
    - [1.3 Zabbix 新型全功能监控软件 Zabbix](#13-zabbix-新型全功能监控软件-zabbix)
  - [2. Cacti 监控平台](#2-cacti-监控平台)
    - [2.1 Cacti 原理](#21-cacti-原理)
    - [2.2 Cacti 监控构建 服务器端(10.10.10.11)](#22-cacti-监控构建-服务器端10101011)
    - [2.3 Cacti 插件添加 10.10.10.12  (客户端)](#23-cacti-插件添加-10101012--客户端)
  - [2. Nagios 监控平台](#2-nagios-监控平台)
    - [2.1 Nagios 原理](#21-nagios-原理)
    - [2.2 Nagios 构建](#22-nagios-构建)
    - [2.3 Nagios 添加监控主机](#23-nagios-添加监控主机)
  - [3. Zabbix 监控构建](#3-zabbix-监控构建)
    - [3.1 初始化系统](#31-初始化系统)
    - [3.2 构建 LAMP 环境](#32-构建-lamp-环境)
    - [3.3 安装配置zabbix-server](#33-安装配置zabbix-server)
    - [3.4 安装配置zabbix-web](#34-安装配置zabbix-web)
    - [3.5 Zabbix 添加监控主机 / 自动发现  10.10.10.12](#35-zabbix-添加监控主机--自动发现--10101012)
    - [3.6 Zabbix Nginx 并发监控(自定义脚本功能)  10.10.10.12](#36-zabbix-nginx-并发监控自定义脚本功能--10101012)
    - [3.7 Zabbix Web场景、组合图](#37-zabbix-web场景组合图)
    - [3.8 Zabbix 报警设置(管理--\> 报警媒介类型--\> 创建媒体类型)](#38-zabbix-报警设置管理---报警媒介类型---创建媒体类型)
      - [3.8.1 邮件报警功能（脚本指定路径查看: /etc/zabbix/zabbix\_server.conf）](#381-邮件报警功能脚本指定路径查看-etczabbixzabbix_serverconf)
    - [3.9 创建动作(触发器)](#39-创建动作触发器)
...
### 3. Zabbix 监控构建
[下载安装Zabbix](https://www.zabbix.com/cn/download?zabbix=7.0&os_distribution=rocky_linux&os_version=9&components=server_frontend_agent_2&db=mysql&ws=nginx)

```bash
# 乱码问题解决

# 第一步：准备中文字体
从你的 Windows 电脑中获取中文字体文件（路径通常为 C:\Windows\Fonts）。推荐选择 宋体（simsun.ttf）或 楷体（simkai.ttf）。将字体文件重命名为英文或拼音，例如：simhei.ttf

# 第二步：上传并替换 Zabbix 字体
登录 Zabbix 服务器，找到网站的字体目录。
- 常见路径（源码安装）：/usr/local/zabbix/php/assets/fonts/ 或 /data/wwwroot/zabbix/assets/fonts/
- 常见路径（RPM/包管理器安装）：/usr/share/zabbix/assets/fonts/
- （老版本可能在 /usr/share/zabbix/fonts/ 或 /usr/share/zabbix/include/fonts/）

将准备好的字体文件上传到该目录下。修改 Zabbix 的字体配置文件以应用新字体：使用文本编辑器打开 /usr/share/zabbix/include/defines.inc.php（或对应的 Zabbix 前端配置文件）。

找到以下两行代码（旧版本或直接使用默认字体名称）：
- define('ZBX_FONT_NAME', 'DejaVuSans');
- define('ZBX_GRAPH_FONT_NAME', 'DejaVuSans');

将 DejaVuSans 替换为上传的字体文件名（不包含 .ttf 或 .ttc 后缀），例如 simhei
```
