...
# ==========================================
expose:
  type: ingress          # 通过 Ingress 暴露
  tls:
    enabled: false       # 学习环境先不开 TLS
  ingress:
    hosts:
      core: harbor.k3s.local       # Harbor 主域名
    annotations:
      # K3s Traefik 注解
...
# Harbor 外部访问地址(重要!镜像 push/pull 用这个)
externalURL: http://harbor.k3s.local
...
persistence:
  enabled: true
  resourcePolicy: "keep"    # 删除 Harbor 不删数据
